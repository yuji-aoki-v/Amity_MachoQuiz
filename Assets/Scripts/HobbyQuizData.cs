...
public class HobbyQuizData : MonoBehaviour
{
    ...
    private void ProcessFetchedData(string jsonData)
    {
        ...
        foreach (JObject doc in documents)
        {
            ...
            string quizText = fields["quiz_json"]?["stringValue"]?.ToString() ?? "No Data";
            string button1 = fields["button1_json"]?["stringValue"]?.ToString() ?? "";
            string button2 = fields["button2_json"]?["stringValue"]?.ToString() ?? "";
            string button3 = fields["button3_json"]?["stringValue"]?.ToString() ?? "";
            string button4 = fields["button4_json"]?["stringValue"]?.ToString() ?? "";
    ...
    {
        string url = $"https://firestore.googleapis.com/v1/projects/amity-4bad3/databases/(default)/documents/hobbyQuiz/{documentId}";  // 修正されたURL
        ...
        JObject updatedData = new JObject
        {
            ["fields"] = new JObject
            {
                ["quiz_json"] = new JObject { ["stringValue"] = inputFields[1].text },
                ["button1_joson"] = new JObject { ["stringValue"] = inputFields[2].text },
                ["button2_joson"] = new JObject { ["stringValue"] = inputFields[3].text },
                ["button3_joson"] = new JObject { ["stringValue"] = inputFields[4].text },
                ["button4_joson"] = new JObject { ["stringValue"] = inputFields[5].text }
            }
        };
